...
static void setup(void)
{
	 ...
	 * clones it to another ring. The ring fds are intentionally inherited
	 * by the forked child: trigger() calls cleanup() in the child to force
	 * IORING_UNREGISTER_BUFFERS after the FOLL_PIN counter has been
	 * drained, which is what triggers the double-unpin WARN/BUG_ON on a
	 * vulnerable kernel. The parent checks taint after the child exits.
	 */
